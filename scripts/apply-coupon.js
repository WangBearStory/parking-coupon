...
async function sendSlackDM(userId, message) {
  ...
  try {
    ...
    if (!msgData.ok) console.log(`Slack 전송 실패 (${userId}): ${msgData.error}`);
  } catch (e) {
...
async function main() {
  ...
  for (const { plate, slackUser } of VEHICLES) {
    const va = await getVehicle(token, plate);
    if (!va) {
      console.log(`${plate}: 미입차 - 스킵`);
      continue;
    }
    if (hasFreeCoupon(va)) {
      console.log(`${plate}: 쿠폰 이미 적용됨 - 스킵`);
      continue;
    }
    const { nextCoupon, count } = await getNextCoupon(token);
    if (!nextCoupon) {
      console.log(`${plate}: 2시간무료 쿠폰 없음 (잔여: ${count})`);
      continue;
    }
    await applyCoupon(token, va, nextCoupon);
    console.log(`${plate}: 2시간무료 쿠폰 적용 완료 (잔여: ${count - 1})`);
    await sendSlackDM(slackUser, `[주차 쿠폰] ${now}\n${plate}: 2시간무료 쿠폰 적용 완료 (잔여: ${count - 1})`);
  }
